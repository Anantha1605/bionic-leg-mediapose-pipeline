...
def visualize_leg_angles(csv_file="output_angles.csv", smooth=False):
    """
    ...
    - smooth: Whether to apply a simple moving average for smoothing
    """
    # Read CSV file
    ...
    # Drop rows with all NaN angle values (if detection failed for some frames)
    df = df.dropna(subset=[
        'right_hip_angle', 'right_knee_angle', 'right_ankle_angle',
        'left_hip_angle', 'left_knee_angle', 'left_ankle_angle'
    ], how='all')
    
    # Extract frame numbers (or use timestamp if preferred)
    frames = df['frame']
    ...
    fig.suptitle("Leg Joint Angle Variation Over Time", fontsize=16, weight='bold')
...
if __name__ == "__main__":
    visualize_leg_angles(csv_file="output_angles.csv", smooth=True)
