...
Features:
 ...
 - Smooths angles with moving average and exponential moving average
...
def main(args):
    ...
    # EMA and Moving Average smoothers
    hip_ema = EMA(alpha=args.smooth)
    knee_ema = EMA(alpha=args.smooth)
    ankle_ema = EMA(alpha=args.smooth)
    hip_ma = MovingAverage(window_size=args.ma_window)
    knee_ma = MovingAverage(window_size=args.ma_window)
    ankle_ma = MovingAverage(window_size=args.ma_window)

    # For simple FPS calc
    prev_time = time.time()

    with mp_pose.Pose(min_detection_confidence=args.detection_conf, min_tracking_confidence=args.tracking_conf) as pose:
        while True:
            ret, frame = cap.read()
            if not ret:
                break
            ...
            # Smooth (combine moving average and EMA)
            hip_ma_val = hip_ma.update(hip_angle)
            knee_ma_val = knee_ma.update(knee_angle)
            ankle_ma_val = ankle_ma.update(ankle_angle)
            hip_s = hip_ema.update(hip_ma_val)
            knee_s = knee_ema.update(knee_ma_val)
            ankle_s = ankle_ema.update(ankle_ma_val)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--ma_window', type=int, default=5, help='Moving average window size')
    parser.add_argument('--detection_conf', type=float, default=0.7, help='MediaPipe min_detection_confidence')
    parser.add_argument('--tracking_conf', type=float, default=0.7, help='MediaPipe min_tracking_confidence')
